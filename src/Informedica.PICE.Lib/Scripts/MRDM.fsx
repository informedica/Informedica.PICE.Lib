...
type MRDMData  = CsvProvider<"../../../mrdm/Export_PICE.csv", ";">

let mrdmData = MRDMData.Load("../../../mrdm/Export_PICE_umcu_20052020.csv")
...
module Codes =
    ...
    let find hn rep name code =
        if code |> String.IsNullOrWhiteSpace then
            if rep then printfn "patient %s has no value for %s" hn name
            ""
        else
            codes
            ...
            | Some (_, _, v) -> v
            | None ->
                printfn "could not find %s code %s" name code
                ""
...
let hospitalAdmissionToString (a : HospitalAdmission) =
    sprintf "Ziekenhuis opname: %A - %A"
        a.HospitalAdmissionDate
        a.HospitalDischargeDate
    |> fun s ->
        a.PICUAdmissions
        |> List.map (picuAdmissionToString >> (sprintf "%s %s") s)


let piceAdmissionToString (a : PICEAdmission) =
    sprintf "%s: %A"
        a.HospitalNumber
        a.PatientState
    |> fun s ->
...
let parseDate s =
    try
        DateTime.ParseExact(s, "d-M-yyyy", CultureInfo.InvariantCulture)
    with
    | _ ->
        sprintf "could not parse %s" s
        |> failwith  
...
let parseDateTime s =
    DateTime.ParseExact(s, "d-M-yyyy hh:mm:ss", CultureInfo.InvariantCulture)
...
let test =
    mrdmData.Rows
    |> Seq.filter (fun r ->
        r.Ziekenhuis_episode_upn |> notNullOrWhiteSpace &&
        r.Gebdat |> notNullOrWhiteSpace    
    )
    |> Seq.groupBy (fun r ->
        r.Ziekenhuis_episode_upn ,
        r.Gebdat,
        r.Status,
        r.Datovl,
        r.Adm_deathmodeid
    )
    |> Seq.map (fun ((hn, bd, st, dod, dm), d) ->
        let find = Codes.find hn
        let st = st |> mapPatientState
        {
            HospitalNumber = hn
            BirthDate = bd |> parseDate
            PatientState = st 
            DateOfDeath = dod |> parseDateOpt
            DeathMode = dm |> find (st = Dead) "Adm_deathmodeid"
            HospitalAdmissions =
                d
                |> Seq.filter (fun r ->
                    let b = r.Adm_hosp_admdate |> notNullOrWhiteSpace
                    if not b then
                        printfn "patient %s doesn't have an hospital admission date" hn 
                    b
                )
                |> Seq.groupBy (fun r ->
                    r.Adm_hosp_admdate,
                ...
                |> Seq.map (fun (( ad, dd), d) ->
                    {
                        HospitalAdmissionDate = ad |> parseDate
                        HospitalDischargeDate = dd |> parseDateOpt
                        PICUAdmissions =
                            d
                            |> Seq.filter (fun r ->
                                let b = r.Adm_ic_admdate |> notNullOrWhiteSpace
                                if not b then
                                    printfn "patient %s has an IC admission without an admission date" hn
                                b
                            )
                            |> Seq.map (fun r ->
                                {
                                    AdmissionDate = r.Adm_ic_admdate   |> parseDate
                                    Urgency = r.Adm_typeid_urgentie    |> mapUrgency
                                    AdmissionType = r.Adm_typeid_soort |> mapAdmissionType      
                                    Recovery = r.Recovery = "1"
                                    AdmissionIndication = r.Adm_indication   |> find true "Adm_indication"
                                    ReferingSpecialism = r.Adm_refspecialism |> find true "Adm_refspecialism"
                                    AdmissionWeight = r.Gewicht    |> parseFloat
                                    AdmissionLength = r.Adm_length |> parseInt
                                    TempMin12 = r.T_min12 |> parseFloat
                                    TempMax12 = r.T_max12 |> parseFloat
                                    RiskDiagnosis = r.Risicodiag_hoofd |> mapRiscDiagnosis
                                    PIM2 = {
                                        PaO2_1 = r.Pao2_0 |> parseFloat
                                        FiO2_1 = r.Fio2_0 |> parseFloat
                                        BE_1 = r.Be_0     |> parseFloat
                                        SBP_1 = r.Sbp_0   |> parseFloat
                                        Ventilated = r.Ventilated = "1"
                                        AdmissionPupils = r.Admpupils |> mapPupils
                                        CardiacByPass = r.Bypass = "1"
                                        CPRPreHosp = r.Cprprehosp_riskpim = "1"
                                        CPRPrePICU = r.Cprprepicu_riskpim = "1"
                                        LeukemiaOrLymphoma = r.Leukemie_riskpim = "1"
                                        BMTransplant = r.Bmtrecipient_riskpim = "1"
                                        SpontCerebralHemorrhage = r.Sponthersenbl_riskpim = "1"
                                        CardioMyopathy = r.Cardmyopath_riskpim = "1"
                                        SCID = r.Scid_riskpim = "1"
                                        HIV = r.Hiv_riskpim = "1"
                                        NeuroDegenerative = r.Neurodegen_riskpim = "1"
                                        HLHS = r.Hypoplast_riskpim = "1"
                                    }
                                }
                            )
                            |> Seq.distinct
...
|> Seq.filter (fun a ->
    ...
    |> Seq.exists (fun ha ->
        ...
        |> Seq.exists (fun pa ->
            let b1 =
                match pa.TempMax12 with
                | Some t -> t >= 38.5
                | None -> false
            let b2 = pa.ReferingSpecialism.Contains("(KON)")
            b2
        )
    )
)
|> Seq.sortBy (fun a -> a.HospitalNumber)
|> Seq.collect piceAdmissionToString
|> Seq.length
//|> Seq.iter (printfn "%s")
